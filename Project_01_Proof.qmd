...
plot.state(Frames_Joined_On_Geo[[2]], "Enrollment_Value")
```

```{r, eval=FALSE, echo=TRUE}
#Plotting for our county data

plot.county <- function(data, state = "FL", top_or_bottom = "top", n = 5, var_name) {
  
  # Ensure the input for top_or_bottom is valid
  if(!top_or_bottom %in% c("top", "bottom")) {
    stop("Invalid value for 'top_or_bottom'. Must be 'top' or 'bottom'.")
  }
  ...
  # Calculate the mean of the specified variable for each Area_name
  mean_values <- state_data %>%
    group_by(area_name) %>%
    summarise(mean_value = mean(get(var_name), na.rm = TRUE)) %>%
    ungroup()
  
  # Sort the Area_names based on the mean values
  if(top_or_bottom == "top") {
    sorted_values <- mean_values %>%
  ...
  # Get the top or bottom n Area_names
  selected_area_names <- sorted_values %>%
    slice(1:n) %>%
    pull(area_name)
  
  # Filter the original data for these Area_names
  final_data <- state_data %>%
    filter(area_name %in% selected_area_names)
  
  return(final_data)
}

plot.county(Frames_Joined_On_Geo[[1]], state = "FL", top_or_bottom = "top", n = 1, var_name = "Enrollment_Value")



  
# Plot the input that will go into the plot.county function once it is running.
  #ggplot(df_filtered, aes(x = year, y = mean_enrollment, color = division))+
  #  geom_line(aes(group = division)) +
  #  geom_point() +
  #  labs(x = "Year", y = "Mean Enrollment Values") + 
